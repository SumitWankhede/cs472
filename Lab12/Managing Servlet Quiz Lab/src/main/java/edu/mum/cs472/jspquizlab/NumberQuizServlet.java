...
public class NumberQuizServlet extends HttpServlet {
	private void genQuizPage(Quiz sessQuiz, PrintWriter out, String currQuest, boolean error, String answer) {
		...
		out.println("<p>" + currQuest.substring(0, currQuest.length() - 2)	+ "<span style= 'color:red'>" + currQuest.charAt(currQuest.length() - 2) + "</span>" + currQuest.charAt(currQuest.length() - 1) + "</p>");
	...
	private void startOverPage(PrintWriter out) {
		...
		out.print("<p>Congratulations: QUIZ is over !></p>");
		out.print("<input type='submit' name='btnRestart' value='Restart' /></p> ");
		out.print("</form>");
		out.print("</body></html>");
	}
